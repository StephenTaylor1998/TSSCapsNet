...
def build_graph(input_shape, num_classes=10, depth=18, tiny=True, half=True):
    if tiny:
        block = TinyBlockDWT
    ...
    if depth == 18:
        model = resnet18_cifar(block=block, num_classes=num_classes, half=half)

    elif depth == 34:
        model = resnet34_cifar(block=block, num_classes=num_classes, half=half)

    elif depth == 50:
        model = resnet50_cifar(block=bottle, num_classes=num_classes, half=half)

    elif depth == 101:
        model = resnet101_cifar(block=bottle, num_classes=num_classes, half=half)

    elif depth == 152:
        model = resnet152_cifar(block=bottle, num_classes=num_classes, half=half)
