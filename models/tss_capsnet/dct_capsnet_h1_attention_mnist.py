...
from ..layers.layers_hinton import PrimaryCaps, Length, Mask
from ..layers.routing import AttentionDigitCaps as DigitCaps
...
def dct_capsnet_graph(input_shape, routing):
    ...
    return tf.keras.Model(inputs=inputs, outputs=[primary, digit_caps, digit_caps_len], name='DCT_CapsNet_Attention')
...
def build_graph(input_shape, mode, n_routing, verbose):
    ...
    if mode == 'train':
        return tf.keras.models.Model([inputs, y_true], [digit_caps_len, x_gen_train],
                                     name='DCT_CapsNet_Attention_Generator')
