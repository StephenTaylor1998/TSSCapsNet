...
from ..layers.operators import Condense, CapsFPN, RoutingTiny, Q, K, CapsuleMapping
...
def efficient_capsnet_graph(input_shape):
    ...
    # (32, 32, 3) ==>> (8, 8, 128)
    x = ResNetBackbone(Bottleneck, [2, 2, 2, 2])(inputs)
    x = tf.keras.layers.BatchNormalization()(x)
    # (8, 8, 128) ==>> (1, 1, 128) ==>> (16, 8)
    x = PrimaryCaps(128, x.shape[1], 16, 8)(x)
    # (16, 8) ==>> (14, 8), (1, 8)
    # x = RoutingTiny()(x)
    # print()
    # feature_pyramid = CapsFPN(out_length=8)(x)
    # q = Q()(feature_pyramid)
    # k = K()(feature_pyramid)
    # print('q', q.shape)
    # print('k', k.shape)
    # x = CapsuleMapping()((k, q, feature_pyramid))
